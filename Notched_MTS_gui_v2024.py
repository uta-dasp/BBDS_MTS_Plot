...
dwell_time.set(30)
notch_len = tk.DoubleVar()
notch_len.set(2.5)
notch_width = tk.DoubleVar()
notch_width.set(2)
...
def select_MTS(prompt):
    ...
    h_by_d = notch_len.get()/width.get()
    h_by_r = notch_len.get()/(notch_width.get()/2)
    sq_h_by_r = math.sqrt(h_by_r)
    C1= 0.721+2.394*sq_h_by_r-0.127*h_by_r
    C2= 1.978-11.489*sq_h_by_r+2.211*h_by_r
    C3= -4.413+18.751*sq_h_by_r-4.596*h_by_r
    C4= 2.714-9.655*sq_h_by_r+2.512*h_by_r
    Kt = C1 +C2 * h_by_d + C3 * h_by_d * h_by_d +C4 * h_by_d * h_by_d * h_by_d

    # print(Kt)
    # Kt= 2.988 -7.3 *h_by_d+9.742*h_by_d*h_by_d-4.429*h_by_d*h_by_d*h_by_d
    ...
    mts2["Stress"] = mts["Force"].apply(
        lambda x: (Kt * x / (thickness.get() * (width.get()-notch_len.get()))) * 1000
    )
...
notch_width_label = tk.Label(root, text="Notch Width (mm)")
notch_width_label.grid(row=2, column=1, columnspan=1)
notch_width_label_entry = tk.Entry(root, textvariable=notch_width)
notch_width_label_entry.grid(row=3, column=1, columnspan=1)
...
notch_len_label = tk.Label(root, text="Notch Length (mm)")
notch_len_label.grid(row=4, column=1, columnspan=1)
notch_len_label_entry = tk.Entry(root, textvariable=notch_len)
notch_len_label_entry.grid(row=5, column=1, columnspan=1)
